...
For ClubSpark, account `a` is the default if you omit `--account`. ClubSpark also requires `--venue`, `--time`, and `--court`.
...
```json
{
  ...
  "booking_open_time": "20:00"
}
```

Important fields:
- `pre_login_seconds`: how early the script logs in before midnight
- `booking_open_time`: when the site opens new bookings, for example `20:00`
...
bash schedule.sh --site club_spark --venue TannerStPark --account a --time 10:00 --court 4
bash schedule.sh --site club_spark --venue TannerStPark --account b --time 11:00 --court 4
```

Pass the target plugin explicitly. For example, if you later add `clubspark_wimbledon`, schedule it with `bash schedule.sh --site clubspark_wimbledon`. For ClubSpark, `--venue`, `--time`, and `--court` are required, while `--account a` and `--account b` install separate LaunchAgents and separate log files.
